...
Because the bookkeeping folders (`pipeline/`, `prds/`, `designs/`, `blog-drafts/`, `reports/`) live inside this same folder, `rh-qs-scaffold` must add them to **the quickstart's own `.gitignore`** (not the factory's) right after initializing the repo, so they never get pushed to the quickstart's public GitHub remote:

```gitignore
# Factory bookkeeping — internal to the quickstart-factory pipeline, not part
# of the shipped quickstart
/pipeline/
/prds/
/designs/
/blog-drafts/
...
This is a **separate concern** from the factory's own `.gitignore`, which simply ignores `.rhoai-qs/` as one blanket rule (see the factory's `.gitignore`) — git never looks inside an ignored directory, so it never notices there's another repo nested inside it, and the two `.gitignore` files don't interact.
