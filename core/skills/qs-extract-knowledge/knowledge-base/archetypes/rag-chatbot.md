...
summary: "Grounds LLM responses in user-uploaded documents via chunking, embedding into a vector store (pgvector, GPU-accelerated Milvus, or inline SQLite-backed Milvus), and similarity search at query time for source-grounded conversational Q&A or embedding-based entity discovery on RHOAI/OpenShift. Choose over agentic-app when core value is document-grounded Q&A without tool calls or multi-step reasoning, over model-serving-app when retrieval beyond direct inference is needed -- six approaches: A (ai-virtual-agent, f5-api-security) lightweight custom RAG with FastAPI+LlamaStack, pgvector, K8s Job ingestion, React/PatternFly or Streamlit, 1-2 GPUs; B (aml-rag-nvidia) NVIDIA RAG Blueprint with NV-Ingest (OCR/table/VLM captioning), GPU-accelerated Milvus, 4 KServe/vLLM models (Nemotron-Super-49B-FP8, VLM, embedding, reranking), embedding/ranking translation proxies bridging NIM API to vLLM, ODF, OpenTelemetry+Grafana+Tempo, 3-5 H100/A100 GPUs with MIG; C (f5-ai-guardrails) security-focused RAG with LlamaStack+pgvector via ai-architecture-charts subcharts, F5 AI Guardrails Moderator proxy for prompt injection/PII/toxicity/topic via OLM operator (SecurityOperator CR ai.security.f5.com/v1alpha1, certified-operators) across 5 namespaces, Llama-3.2-1B-Instruct + all-MiniLM-L6-v2, dual-panel Streamlit UI, Calypso AI Red Team; D (RAG) enterprise RAG with Kubeflow Pipelines multi-source ingestion (5 domains), Docling+inline PyPDF document processing, multi-device serving (GPU/HPU/Xeon/CPU), dual Direct/Agent-based modes with MCP+Tavily, LlamaStack safety shields (Llama Guard), DeepEval evaluation (faithfulness/contextual precision/relevancy/ConversationalGEval), ArgoCD multi-tenant bootstrap; E (llm-cpu-serving) CPU-only RAG via LlamaStackDistribution CR (llamastack.io/v1alpha1) with inline Milvus (SQLite-backed), inline sentence-transformers (granite-embedding-125m-english), TinyLlama-1.1B on CPU-only vLLM, AnythingLLM workbench, K8s Job URL-based document seeding, no GPU or custom backend; F (peoplemesh) semantic search talent discovery with Quarkus+LangChain4j backend, Docling (IBM Research) for intelligent resume parsing (PDF/DOCX/images), pgvector for embedding-based people search, dual model serving via Ollama (CPU, granite4:3b + granite-embedding:30m 384D) or vLLM/KServe (GPU, Qwen2.5-7B-AWQ), Red Hat build of Keycloak for OIDC authentication, Flyway database migrations, containerized installer. Approach B requires `APP_VECTORSTORE_ENABLEGPUINDEX: \"True\"` and `APP_VECTORSTORE_ENABLEGPUSEARCH: \"True\"` in charts/ingest/values.yaml for GPU Milvus plus embedding/ranking translation proxies bridging NVIDIA NIM API to vLLM; Approach C routes requests through F5 Moderator evaluating active guardrail policies before forwarding to LlamaStack, blocking violations with `cai_error` response containing `scanner_results`; Approach D configures device type via `global.models` with device-specific args and toggles 6 ai-architecture-charts subcharts via condition flags in umbrella chart; Approach E defines full LlamaStack configuration in a ConfigMap including remote vLLM inference, inline sentence-transformers, inline Milvus vector_io, and file-search tool runtime; Approach F toggles `llm.mode` in charts/peoplemesh/values.yaml between `ollama.enabled` (CPU default) and `vllm.enabled` (GPU) with `EMBEDDING_DIMENSION` configurable per mode. Approach B requires NGC API key for cloud-hosted NV-Ingest NIMs (document detection/OCR not locally served); Approach C requires F5 license key + private registry credentials (harbor.calypsoai.app), anyuid SCC bindings plus pre-applied inference model SCC (`openshift-inference-models-scc.yaml`), Helm retry logic (`F5_HELM_MAX_ATTEMPTS=8`, `F5_HELM_RETRY_SLEEP=6`) for namespace races, and persists guardrail state to emptyDir JSON file (`F5_GUARDRAILS_STATE_FILE=/data/guardrails_state.json`) lost on pod replacement -- use PVC for persistence; Approach D requires min 16vCPU/64GB RAM for Xeon CPU-only deployment; Approach E stores all state (vector DB, metadata, KV store) in SQLite files within pod filesystem, lost on restart unless backed by a PVC; Approach F requires Keycloak Operator pre-installed, CPU-only Docling resume processing takes 2-3 minutes per upload vs 10-20 seconds with GPU."
metadata:
  type: archetype
tags:
  tech_stack: [fastapi, react, patternfly, postgresql, python, nvidia-rag-blueprint, redis, streamlit, llama-stack, ansible, docling, deepeval, kubeflow-pipelines, anythingllm, sentence-transformers, quarkus, langchain4j, ollama, keycloak]
  ai_pattern: [rag, embeddings, vector-search, model-serving, multimodal, guardrails, evaluation, data-pipeline, agents, semantic-search, document-processing]
...
source_examples:
  ...
  - quickstart: "peoplemesh"
    ...
    notes: "Semantic search talent discovery platform using Docling (IBM Research) for resume parsing, Quarkus+LangChain4j backend, pgvector for embedding-based people search, Ollama (CPU) or vLLM/KServe (GPU) dual model serving, and Keycloak enterprise authentication"
    approach: "F"
---
...
| peoplemesh | AI-powered talent discovery using embedding-based semantic search over resume profiles -- Docling (IBM Research) for intelligent resume parsing (PDF/DOCX/images), Quarkus+LangChain4j backend with React SPA, pgvector for vector similarity search, dual model serving via Ollama (CPU, granite4:3b + granite-embedding:30m) or vLLM/KServe (GPU, Qwen2.5-7B-AWQ), and Red Hat build of Keycloak for enterprise OIDC authentication |
...
## Approach F: Semantic Search Talent Discovery Platform (from peoplemesh)

### When to Use

When the application uses embedding-based vector similarity search to find entities (people, products, resources) by meaning rather than keywords, with intelligent document processing for structured data extraction, and the primary user interaction is search/discovery rather than conversational Q&A -- the retrieval infrastructure (embeddings + vector DB) serves a search use case, not a chatbot.

### Differences from Approach A

- **User interaction model:** Approach A is conversational Q&A -- users ask questions and receive LLM-generated answers grounded in retrieved documents; Approach F is semantic search -- users enter search queries (e.g., "mobile developer in Italy") and receive ranked entity results (people profiles) directly, with no LLM-generated answers from retrieved content (`README.md` data flow: "User searches 'mobile developer' -> LLM parses intent -> Vector similarity search -> Ranked results")
- **LLM role:** Approach A uses the LLM to generate answers from retrieved context (retrieval-augmented generation); Approach F uses the LLM for query intent parsing and profile structuring during document ingestion, not for answer generation -- the search results are displayed directly without LLM synthesis
- **Backend technology:** Approach A uses Python/FastAPI with LlamaStack; Approach F uses Java/Quarkus REST with LangChain4j (`QUARKUS_LANGCHAIN4J_OPENAI_TIMEOUT` in `charts/peoplemesh/templates/config-map.yaml`), connecting to the LLM via OpenAI-compatible API (`OPENAI_BASE_URL`)
- **Frontend:** Approach A uses React/PatternFly within an agent platform; Approach F serves a standalone React SPA with a search interface for people discovery, profile viewing, and resume upload (`quay.io/rh-ai-quickstart/peoplemesh` container image)
- **Document processing:** Approach A uses a K8s Job or background task for simple document chunking; Approach F uses Docling (IBM Research, `quay.io/docling-project/docling-serve-cpu` or GPU variant `quay.io/docling-project/docling-serve`) for intelligent resume parsing with layout-aware text extraction from PDFs, DOCX, and scanned images, connected via REST API (`QUARKUS_DOCLING_BASE_URL` in ConfigMap)
- **Dual model serving:** Approach A deploys vLLM/LlamaStack for GPU-based inference; Approach F supports two interchangeable model serving modes toggled via `llm.mode` in `charts/peoplemesh/values.yaml` -- Ollama (`ollama.enabled: true`, default, CPU-only) serving `granite4:3b` for chat and `granite-embedding:30m` (384 dimensions) for embeddings from a single endpoint (`ollama-service:11434/v1`), or vLLM via KServe InferenceService (`vllm.enabled: true`, GPU) serving Qwen2.5-7B-Instruct-AWQ with RawDeployment mode and `--quantization awq` args
- **Embedding model:** Approach A relies on LlamaStack or a shared model for embeddings; Approach F uses dedicated embedding models -- `granite-embedding:30m` (384D) for Ollama mode or the same Qwen2.5-7B for vLLM mode (`EMBEDDING_DIMENSION` in ConfigMap), with the embedding dimension configurable per mode
- **Authentication:** Approach A has no authentication; Approach F includes Red Hat build of Keycloak (`k8s.keycloak.org/v2alpha1 Keycloak` CR in `charts/keycloak/templates/keycloak-cr.yaml`) with a `peoplemesh` realm, OIDC client, KeycloakRealmImport CR for automated realm provisioning, and multi-IDP support (Google, Microsoft, LDAP) -- client secret and issuer URL are auto-synced to the PeopleMesh app via a post-install secrets-sync Job (`charts/peoplemesh/templates/secrets-sync-job.yaml`)
- **Helm chart:** Approach A uses custom Helm or ai-architecture-charts subcharts; Approach F uses a self-contained umbrella chart (`peoplemesh-umbrella/Chart.yaml`) with 6 conditional local dependencies: `keycloak`, `pgvector`, `docling`, `vllm`, `ollama`, and `peoplemesh` -- each with a `condition` toggle (e.g., `vllm.enabled`, `ollama.enabled`)
- **Database migrations:** Approach A has no schema migration tooling; Approach F uses Flyway with configurable migration locations (`QUARKUS_FLYWAY_LOCATIONS` in ConfigMap) supporting separate migration and seed data paths (`classpath:db/migration,classpath:db/granite` or `classpath:db/migration,classpath:db/openai`) for provider-specific schema initialization
- **GPU flexibility:** Approach A requires 1-2 GPUs; Approach F defaults to CPU-only (Ollama) with optional GPU acceleration for both Docling and LLM -- GPU mode requires only a single NVIDIA GPU with 16GB+ VRAM (A10G, T4, V100) per `README.md`; CPU-only mode works but resume processing takes 2-3 minutes per upload vs 10-20 seconds with GPU
- **Installer:** Approach F includes a containerized installer (`installer/Dockerfile`, UBI9-based) bundling the Helm umbrella chart, `oc`, `helm`, and deployment scripts for automated one-command deployment
- **Organization/compliance configuration:** Approach F includes GDPR-oriented organization settings (`ORGANIZATION_NAME`, `ORGANIZATION_CONTACT_EMAIL`, `ORGANIZATION_DPO_EMAIL`, `ORGANIZATION_DATA_LOCATION`, `ORGANIZATION_GOVERNING_LAW` in ConfigMap) not present in other approaches

### Typical Components

- **Model serving:** Ollama (default, CPU-only, `ollama/ollama:latest`) serving `granite4:3b` (chat) and `granite-embedding:30m` (384D embeddings) from a single endpoint with PVC-backed model storage; alternatively KServe InferenceService with vLLM ServingRuntime (`quay.io/redhat-ai-dev/vllm-openai-ubi9`) serving Qwen2.5-7B-Instruct-AWQ via HuggingFace URI storage with RawDeployment mode and 1 NVIDIA GPU
- **Backend:** Quarkus REST application (`quay.io/rh-ai-quickstart/peoplemesh`) with LangChain4j for LLM integration, OpenAI-compatible API access, Flyway database migrations, resume processing pipeline (Docling -> LLM structuring -> embedding storage), and semantic search API
- **Frontend:** React SPA (served by Quarkus) with natural language search interface, profile display, resume upload, and geographic/skill-based filtering
- **Data layer:** PostgreSQL 15 + pgvector (`quay.io/rh-aiservices-bu/postgresql-15-pgvector-c9s`) for profile data storage, vector embeddings, and similarity search; PVC-backed with 20Gi default
- **Supporting:** Docling (IBM Research, `quay.io/docling-project/docling-serve-cpu` or GPU variant) for intelligent resume parsing (PDF/DOCX/images) with REST API and optional UI; Red Hat build of Keycloak (`k8s.keycloak.org/v2alpha1`) with realm import, OIDC client auto-provisioning, and multi-IDP support; umbrella Helm chart with conditional dependencies; containerized installer for automated deployment

---

## Choosing Between Approaches

| Criteria | Approach A (Custom-built) | Approach B (NVIDIA Blueprint) | Approach C (Guardrails-Secured) | Approach D (Enterprise RAG with Pipelines) | Approach E (Lightweight Inline RAG) | Approach F (Semantic Search Platform) |
|----------|--------------------------|-------------------------------|--------------------------------|-------------------------------------------|-------------------------------------|--------------------------------------|
| RAG pipeline | Custom FastAPI + LlamaStack | NVIDIA RAG Server pre-built container | LlamaStack (ai-architecture-charts subchart) | LlamaStack (ai-architecture-charts subchart) with Kubeflow Pipelines for ingestion | Llama Stack Distribution CR with inline providers (no custom backend) | Quarkus + LangChain4j with Docling document processing and pgvector semantic search (no LLM answer generation) |
| Document processing | Simple chunking pipeline | NV-Ingest with OCR, table/graphic detection, VLM captioning | LlamaStack RAG tool chunking | Kubeflow Pipelines multi-source ingestion + Docling PDF processing + LlamaStack inline PyPDF | K8s Job fetching URLs + HTML stripping via llama-stack-client | Docling (IBM Research) for intelligent resume parsing (PDF/DOCX/images) with layout-aware text extraction |
| Vector database | pgvector (CPU) | Milvus (GPU-accelerated) | pgvector (CPU) | pgvector (CPU) | Inline Milvus (SQLite-backed, no separate deployment) | pgvector (CPU) |
| Models required | 1-2 (LLM + optional embedding) | 4 (LLM, VLM, embedding, reranking) | 1 LLM + 1 embedding (Llama-3.2-1B + MiniLM) | 1 LLM + 1 embedding (configurable) + optional safety shield | 1 LLM (TinyLlama 1.1B) + 1 inline embedding (granite-embedding-125m-english) | 1 LLM + 1 embedding (granite4:3b + granite-embedding:30m via Ollama, or Qwen2.5-7B-AWQ via vLLM) |
| GPU requirements | Low (1-2 GPUs) | High (3-5 H100/A100 GPUs, MIG optional) | Low (1 GPU for LLM) + dedicated GPU for F5 guardrail scanners | Flexible (1 GPU, or HPU, or Xeon CPU, or CPU-only) | None (CPU only, min 2 cores / 4Gi) | None (CPU default via Ollama) or Low (1 GPU with 16GB+ VRAM for vLLM + optional GPU for Docling) |
| AI security | None | None | F5 AI Guardrails (prompt injection, PII, toxicity, topic) | LlamaStack safety shields (Llama Guard, configurable input/output shields) | None | None |
| External dependencies | None | NGC API key for cloud-hosted NV-Ingest NIMs | F5 license key + private registry credentials | Tavily API key (optional, for web search) | None | Red Hat build of Keycloak Operator (required for authentication) |
| Namespace topology | Single namespace | Single namespace | 5 namespaces (RAG + 4 F5 operator-managed) | Single namespace (multi-tenant via ArgoCD bootstrap) | Single namespace | Single namespace |
| Frontend | React/PatternFly | React (NVIDIA Blueprint UI) | Streamlit (dual-panel guardrailed vs direct) | Streamlit (dual Direct/Agent-based modes with MCP tools) | AnythingLLM workbench + Llama Stack playground | React SPA with search interface, profile display, and resume upload |
| Observability | Not included | Full stack (OpenTelemetry, Grafana, Tempo) | F5 Moderator dashboard (allowed/blocked counts, audit logs) | Not included (evaluation framework for offline quality assessment) | Not included | Not included |
| Evaluation | Not included | Not included | Not included | DeepEval (faithfulness, contextual precision/relevancy, chunk count, conversational G-Eval) | Not included | Not included |
| Agent capabilities | None | None | None | LlamaStack Agent mode with MCP tools, web search, file search | File-search tool runtime + MCP protocol support (via Llama Stack Distribution) | None |
| Multi-device support | GPU only | GPU only (NVIDIA H100/A100) | GPU only | GPU, Intel Gaudi HPU, Intel Xeon CPU, generic CPU | CPU only (Intel, AVX512 preferred) | CPU (Ollama default) or GPU (vLLM/KServe) |
| Multi-tenancy | Not included | Not included | Not included | ArgoCD-based tenant bootstrap chart | Not included | Not included |
| Authentication | Not included | Not included | Not included | Not included | Not included | Red Hat build of Keycloak (OIDC/SAML, multi-IDP, realm auto-provisioning) |
| Backend technology | Python/FastAPI | Python (NVIDIA RAG Server) | Python/LlamaStack | Python/LlamaStack + Streamlit | No custom backend (Llama Stack Distribution CR) | Java/Quarkus + LangChain4j |
| Best for | Lightweight RAG within a broader app | Enterprise document-heavy RAG with GPU resources | Demonstrating AI-layer security for model inference endpoints | Full-featured enterprise RAG reference implementation with pipeline-based ingestion, multi-device flexibility, and built-in evaluation | Quick zero-GPU RAG demo with no custom code, inline vector DB, and out-of-the-box chat UI | Embedding-based entity search/discovery (talent, products, resources) with intelligent document processing and enterprise authentication |
