...
summary: "Provides a single Makefile entry point for managing Helm values file lifecycle (copy-from-example, HF token prompting, in-place sed updates) and dynamic --set flag assembly from CLI variables, supporting both interactive developer and scripted CI installation flows for a RAG stack (pgvector, llamastack, Streamlit) with Ansible-delegated F5 XC infrastructure deployment. Use when you need one Makefile to handle values file initialization, interactive credential prompting, and dynamic model/device/toleration CLI overrides via helm upgrade --install --set flags -- unlike makefile-split-cluster-local-interactive-env which uses external shell scripts for env collection instead of inline define blocks. Two inline define blocks drive the flow: check_values_file copies rag-values.yaml.example and pauses for editing (skipped when LLM is set), while validate_values_file extracts the HF token via grep/sed range pattern /^llm-service:/,/^[^ ]/ and prompts/updates in-place; CLI variables (LLM, SAFETY, DEVICE, LLM_TOLERATION, EXTRA_HELM_ARGS, RAW_DEPLOYMENT, LLAMA_STACK_ENV) are conditionally appended as --set flags. A TOLERATIONS_TEMPLATE macro is defined for --set-json but the install target uses individual --set flags instead; oc delete jobs --all must run pre-install to clear completed init jobs blocking Helm upgrade; the values file is gitignored to prevent credential commits; and the sed YAML range pattern assumes standard indentation in the values file."
metadata:
