...
summary: "Automates F5 Distributed Cloud Customer Edge mesh deployment on OpenShift via four Ansible roles (ocp_preflight, hugepages, storage_validation, f5xc_mesh) using kubernetes.core>=3.0.0, handling cluster validation, HugePages kernel tuning via Tuned/MCP, storage class checks, and mesh lifecycle including Prometheus hostPort conflict resolution controlled by fix_prometheus_hostport (default true). Use when deploying F5 XC CE infrastructure requiring HugePages and automated Prometheus patching on OpenShift; the Makefile f5-deploy target wraps ansible-playbook with tagged stages (step1: preflight/hugepages/storage, step2: mesh) and ANSIBLE_TAGS for selective execution, complementing Helm-based application deployment; requires oc CLI logged in and secrets.yml with f5xc_cluster_name/f5xc_token created from secrets.yml.example. Nodes auto-discovered via kubernetes.core.k8s_info if ocp_node_name is unset and labeled worker-hp for a Tuned profile MachineConfigPool (1792x2M pages); mesh renders CE manifests via Jinja2 or static YAML (f5xc_manifest_source) in ves-system namespace, waits for PVC binding (120s) and pod readiness (2100s/35min), polls for manual F5 XC Console site approval (600s), then patches Prometheus via oc get | python3 | oc replace to strip hostPort entries. Site approval requires manual action in the F5 XC Console and cannot be automated, HugePages requires a node reboot the playbook warns about but does not wait for, the Prometheus fix uses python3 piping rather than strategic merge patch because JSON patches cannot remove individual keys within lists, and cleanup preserves HugePages by default -- pass -e remove_hugepages=true or make f5-clean REMOVE_HUGEPAGES=1 to also remove Tuned/MCP/labels."
metadata:
...
source_examples:
  ...
  - quickstart: "f5-api-security"
    ...
    notes: "Same four Ansible roles deploying F5 XC CE mesh with auto-approve site registration and Prometheus hostPort fix"
    approach: "A"
---
