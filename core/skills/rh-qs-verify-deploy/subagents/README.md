# Verify-Deploy Subagents

This directory contains the `validation-skill` subagent used by `rh-qs-verify-deploy` ([../SKILL.md](../SKILL.md)) to resolve which quickstart a session applies to before running verification.
...
| **When used** | Phase 0 — before reading the design doc or writing the verification report |
...
Since `rh-qs-verify-deploy` runs inside `.rhoai-qs/<slug>/` itself (the scaffolded quickstart repo), the existing-slugs listing comes from `../`, one level up — that's `.rhoai-qs/` itself.
