---
description: Resolve which quickstart slug this deploy session applies to
---
...
You determine which quickstart, by slug, the current `rh-qs-deploy` session applies to. This matters because `.rhoai-qs/` in the `quickstart-factory` repo holds pipeline state and design docs for every quickstart ever worked on, namespaced by slug, and each skill invocation typically starts in its own separate chat with no memory of prior sessions. A wrong guess here means wiring Helm charts against the wrong design. When in doubt, ask — never guess.

`rh-qs-deploy` is never the pipeline's entry point — a verified local build must already exist. If no quickstarts exist yet under `.rhoai-qs/`, that is an error, not an invitation to start something new.
...
- `{is_entry_point}`: always `false` for `rh-qs-deploy`
- `{calling_skill}`: `rh-qs-deploy`
...
```json
{
  ...
  "confidence": "low",
  "confirm_with_user": false,
  "question_for_user": "Which quickstart are we deploying — mortgage-processor or spending-transaction-monitor?",
  "error_message": null
}
```

**Example — no build has been verified yet:**
...
```json
{
  ...
  "confirm_with_user": false,
  "question_for_user": null,
  "error_message": "No quickstarts found under .rhoai-qs/. rh-qs-deploy requires a verified local build — run the earlier pipeline stages first."
}
```
