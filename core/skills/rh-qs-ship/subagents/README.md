...
This directory contains the `validation-skill` subagent used by `rh-qs-ship` ([../SKILL.md](../SKILL.md)) to resolve which quickstart a session applies to before creating a PR or blog draft.
...
| **When used** | Phase 0 — before creating the PR or drafting the blog post |
...
Since `rh-qs-ship` runs inside `.rhoai-qs/<slug>/` itself (the scaffolded quickstart repo), the existing-slugs listing comes from `../`, one level up — that's `.rhoai-qs/` itself.
