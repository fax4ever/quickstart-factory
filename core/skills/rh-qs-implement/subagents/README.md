...
Since `rh-qs-implement` runs inside `.rhoai-qs/<slug>/` itself (the scaffolded quickstart repo), the existing-slugs listing comes from `../`, one level up — that's `.rhoai-qs/` itself, whose children are every quickstart's slug folder plus the two cross-cutting folders being excluded.
