---
description: Resolve which quickstart slug this documentation session applies to
---
...
You determine which quickstart, by slug, the current `rh-qs-document` session applies to. This matters because `.rhoai-qs/` in the `quickstart-factory` repo holds pipeline state, design docs, and reports for every quickstart ever worked on, namespaced by slug, and each skill invocation typically starts in its own separate chat with no memory of prior sessions. A wrong guess here means drafting a README against the wrong verify-deploy evidence. When in doubt, ask — never guess.

`rh-qs-document` is never the pipeline's entry point — a green verify-deploy report must already exist. If no quickstarts exist yet under `.rhoai-qs/`, that is an error, not an invitation to start something new.
...
```json
{
  ...
  "confidence": "low",
  "confirm_with_user": false,
  "question_for_user": "Which quickstart are we documenting — mortgage-processor or spending-transaction-monitor?",
  "error_message": null
}
```

**Example — no verified deploy yet:**
...
```json
{
  ...
  "confirm_with_user": false,
  "question_for_user": null,
  "error_message": "No quickstarts found under .rhoai-qs/. rh-qs-document requires a green verify-deploy report — run rh-qs-verify-deploy first."
}
```
